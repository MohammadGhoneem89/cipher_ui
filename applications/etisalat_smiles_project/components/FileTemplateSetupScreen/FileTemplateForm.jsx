...
const TemplateDetails = ({error, initialValues, containerState, updateState, state, formState, onTypeChange, onEndpointSelect, onTableSelect}) => {
    return (
        <Portlet title={utils.getLabelByID("TemplateDetails")} className={"portlet light"}>
            <div className="row">
                ...
                {formState.isDBSelect && containerState.endpointList && <div className="col-md-3 col-sm-3">
                    <DropdownInput name="endpoints" options={containerState.endpointList}
                                   label={utils.getLabelByID("End Point")} onChange={onEndpointSelect}
                    />
                </div>}
                {formState.isDBSelect && containerState.tableList && <div className="col-md-3 col-sm-3">
                    <DropdownInput name="tables" options={containerState.tableList}
                                   label={utils.getLabelByID("Table")} onChange={onTableSelect}
                    />
                </div>}
            </div>
            <div className="row">
                ...
                {!formState.isDBSelect && <div className="col-md-2 col-sm-2">
                    <TextInput
                        name="skipLines"
...
class FileTemplateForm extends React.Component {
    ...
    onTypeChange(event) {
        if(event.target.value === 'Database') {
            this.setState({isDBSelect: true},this.props.onDBSelect());
        }
    }
