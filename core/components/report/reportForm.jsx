...
const ReportForm = ({ onInputChange, addPeer, state, ActionHandlers, flag, isOwner, onSubmit, testQuery, startDateChange, addvalue, removevalue }) => {
  ...
  if (state.queryCount > 0) {
    for (let i = 0; i < state.queryCount; i++) {
      viewBag.push(
        <div className="row" style={{ marginBottom: "10px" }}>
          <div className="col-md-12">
            <textarea key={i} type="text" disabled={!isOwner} className="form-control" id={`queryStrMulti-${i}`} rows="4"
              value={_.get(state.reportContainer, `queryStrMulti-${i}`, '')}
              onChange={onInputChange}
              style={{ resize: "none", width: "100%", marginBottom: "10px" }} />
            <button type="submit" onClick={testQuery.bind(this, i)}
                ...
                <div className="col-md-12">
                  ...
                  <div className="row">
                    <div className="col-md-12">
                      <div className="form-group">
                        <div className="form-group col-md-12">
                          {state.reportContainer.reportType != 'graphic-bar-multi' &&
                            <div>
                          ...
                          {state.reportContainer.reportType == 'graphic-bar-multi' &&
                            <div>
                              <label className="form-group control-label " style={{
                                textAlign: "left",
                                fontWeight: "normal"
                              }}>{utils.getLabelByID("SQl Query for labels")}</label>
                              <textarea type="text" disabled={!isOwner} className="form-control" id="queryStrLabel" rows="4"
                                value={state.reportContainer.queryStrLabel}
                                onChange={onInputChange}
                                style={{ resize: "none", width: "100%" }} />
                              <div className="row" style={{ marginTop: "10px" }} >
                                <div className="col-md-12">
                                  <div className="btn-toolbar pull-right" >
                                    <a className="btn btn-default pull-right" onClick={addvalue}> <i className="fa fa-plus" aria-hidden="true"></i> Add Value Query</a> &nbsp;
                                <a className="btn btn-default pull-right" onClick={removevalue}> <i className="fa fa-minus" aria-hidden="true"></i> Remove last value query</a>
                                    <a onClick={testQuery.bind(this, -1)}
                                      className="btn btn-default pull-right">{' '}
                                      <i className="fa fa-flask" aria-hidden="true"></i>
                                      {utils.getLabelByID("  Simulate Query")}
                                    </a>
                              ...
                              <div>
                                <label className="form-group control-label " style={{
                                  textAlign: "left",
                                  fontWeight: "normal"
                                }}>{utils.getLabelByID("SQl Query for Values")}</label>
                                {viewBag}
                              </div>
                            </div>
                          }
